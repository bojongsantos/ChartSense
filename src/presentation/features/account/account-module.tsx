...
export function AccountModule({ user, premiumPriceIdr }: { user: CurrentUserDto; premiumPriceIdr: number }) {
  ...
  return (
    <div className="mx-auto flex max-w-3xl flex-col gap-5 p-6">
      <div><h1 className="text-xl font-bold">Akun & Billing</h1><p className="mt-1 text-sm text-muted">Kelola identitas dan paket ChartSense.</p></div>
      ...
      <section className="card p-5"><div className="flex items-center gap-2"><Crown className="size-5 text-warning" /><h2 className="text-sm font-bold">ChartSense Premium</h2></div><p className="mt-2 text-sm text-muted">Akses scanner penuh, signals, dan watchlist hingga 200 simbol selama 30 hari.</p><p className="mt-4 text-2xl font-bold">{new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", maximumFractionDigits: 0 }).format(premiumPriceIdr)} <span className="text-xs font-normal text-muted">/ 30 hari</span></p>{user.plan === "PREMIUM" ? <p className="mt-4 text-sm font-semibold text-positive">Premium aktif.</p> : <button onClick={checkout} disabled={loading} className="mt-4 inline-flex items-center gap-2 rounded-lg bg-gradient-to-r from-accent to-accent-blue px-4 py-2.5 text-sm font-bold text-white disabled:opacity-60">{loading && <Loader2 className="size-4 animate-spin" />}Upgrade melalui Midtrans</button>}{error && <p className="mt-3 text-xs text-negative">{error}</p>}</section>
