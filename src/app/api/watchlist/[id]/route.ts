...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await prisma.watchlistItem.updateMany({ where: { id, userId: user.id, position: { lt: limit } }, data: input });
...
export async function DELETE(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const limit = getWatchlistLimit(user.plan);
    const { id } = await context.params;
    const result = await prisma.watchlistItem.deleteMany({ where: { id, userId: user.id, position: { lt: limit } } });
    if (!result.count) throw new HttpError(404, "Item watchlist tidak ditemukan.", "NOT_FOUND");
